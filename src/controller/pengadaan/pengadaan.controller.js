...
const pengadaanModel = require("../../model/pengadaan/pengadaan.model");
...
const pengadaanController = {
  ...
  getData: (req, res) => {
    ...
    /*** Error Temporary validation for pengadaan */
    /**** No Surat validation */
    if (!req.body.noSurat) {
      return res
        .status(403)
    ...
    if (req.body.bentukKontrak == 1 || req.body.bentukKontrak == 2) {
      return res
        .status(403)
        .json({ error: "Bentuk Kontrak for APBD must be 3, 4 or 5" });
    }
    ...
    return (
      pengadaanModel
        ...
        .then((result) => {
          ...
          return Promise.all(
            id.map((idBast) => pengadaanModel.getRincian(idBast)) //maping the array contain idBast that used in model
          ).then((rincianResult) => ({
            result,
